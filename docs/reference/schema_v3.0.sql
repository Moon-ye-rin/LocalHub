...
--     ※ 이유: areacode는 79%(5,148/6,518), cat1~3은 78%가 빈 문자열이라
--        명세서 5.9의 areacode 필터는 실데이터에서 거의 동작하지 않는다.
CREATE TABLE locations (
    contentid     VARCHAR(20)  PRIMARY KEY,        -- 원본 고유 ID (6,518건 전부 유니크 확인)
...
-- keyword 검색(title/addr1 LIKE '%..%')은 인덱스를 못 탄다. 6,518건이라 무시 가능.
...
--   POST /api/posts/{id}/like        post_likes INSERT OR IGNORE → like_count 자동
--   GET  /api/tags                   tags + post_tags COUNT
--   GET  /api/locations              locations (contenttypeid/keyword/페이지네이션)
--   GET  /api/locations/{contentid}  locations PK 조회
